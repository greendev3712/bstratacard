...
namespace Lib
{
	...
	public class MultiColumnComboPopup : System.Windows.Forms.Form
	{
		...
		{
			int i, w = 0;
			for (i = 0; i < lstvMyView.Columns.Count; i++)
			{
				w += lstvMyView.Columns[i].Width;
			}
			return w; //Math.Min(400, w);
		}

		private void resizeColumnsToHeadersAndContent()
		{
			int i;
			int[] widths = new int[lstvMyView.Columns.Count];

			lstvMyView.AutoResizeColumns(ColumnHeaderAutoResizeStyle.ColumnContent);
			for (i = 0; i < lstvMyView.Columns.Count; i++)
				widths[i] = lstvMyView.Columns[i].Width;

			lstvMyView.AutoResizeColumns(ColumnHeaderAutoResizeStyle.HeaderSize);
			for (i = 0; i < lstvMyView.Columns.Count; i++)
				if (widths[i] > lstvMyView.Columns[i].Width)
					lstvMyView.Columns[i].Width = widths[i];

		}

		public void autoSize(int itemHeight)
		{
			const int MccbMaxNumVisibleItems = 15;
			const int MccbMaxWidth = 640;

			/// @todo get below constants programatically from .NET
			const int MccbItemExtraHeight = 1; // gridlines==true?
			const int MccbHeaderExtraHeight = 4;
			const int MccbDropdownExtraHeight = 1;
			const int MccbVerticalScrollbarWidth = 16;
			const int MccbHorizontalScrollbarHeight = 16;

			int newWidth = getListWidth();
			int newHeight = 0;

			if (newWidth > MccbMaxWidth)
			{
				newWidth = MccbMaxWidth;
				newHeight = MccbHorizontalScrollbarHeight;
			}

			if (MccbMaxNumVisibleItems < dataTable.Rows.Count)
			{
				newHeight += MccbMaxNumVisibleItems * (MccbItemExtraHeight + itemHeight) + itemHeight + MccbHeaderExtraHeight + MccbDropdownExtraHeight;
				// add some width for the vertical scrollbar
				newWidth += MccbVerticalScrollbarWidth;
			}
			else
				newHeight += dataTable.Rows.Count * (MccbItemExtraHeight + itemHeight) + itemHeight + MccbHeaderExtraHeight + MccbDropdownExtraHeight;
		...
		public MultiColumnComboPopup(DataRow[] drows)
		{
			...
			resizeColumnsToHeadersAndContent();
